...
class RayPPOTrainer:
    ...
    def _validate(self, merged: bool = False, valid_idx = 0):
        data_source_lst = []
        ...
        for test_data in self.val_dataloader:   
            ...
            if not self.async_rollout_mode:
                print("actor_rollout_wg:", type(self.actor_rollout_wg))
                print("generate_sequences file:", inspect.getfile(self.actor_rollout_wg.generate_sequences))
                test_output_gen_batch_padded = self.actor_rollout_wg.generate_sequences(test_gen_batch_padded)
            else:
                print("async_rollout_manager:", type(self.async_rollout_manager))
                print("generate_sequences file:", inspect.getfile(self.async_rollout_manager.generate_sequences))
                test_output_gen_batch_padded = self.async_rollout_manager.generate_sequences(test_gen_batch_padded, is_valid=True)
            ...
            os.makedirs(f"/share/nlp/baijun/shuhan/val_output_rollout-8/Llama3.2-3B-Instruct_Val_format-0_savepoint/{valid_idx}", exist_ok=True)
            for j in range(len(input_texts)):
                ...
                with open(f"/share/nlp/baijun/shuhan/val_output_rollout-8/Llama3.2-3B-Instruct_Val_format-0_savepoint/{valid_idx}/output_{j+1}.json", "w", encoding="utf-8") as f:
                    f.write(json.dumps(
                output_content,
                ensure_ascii=False,
                indent=2
            ))
            #print("write response in doc")
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            val_metrics = self._validate(valid_idx=0)
            ...
            for batch_dict in self.train_dataloader:
                ...
                if (
                    ...
                    with marked_timer("testing", timing_raw, color="green"):
                        val_metrics: dict = self._validate(valid_idx=valid_idx)
                        valid_idx += 1
                        if is_last_step:
