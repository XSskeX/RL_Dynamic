...
class Explainer(ABC):
    ...
    def _highlight(self, str_toks: list[str], activations: list[float], sentence_level: bool) -> str:
        ...
        if sentence_level:
            ...
            while i < len(str_toks):
                if str_toks[i] == '[CLS]' or str_toks[i] == '[PAD]':
                    i += 1
                    continue
                result += str_toks[i]
                i += 1
    ...
    def _join_activations(
        ...
        acts = ""
        if sentence_level:
            acts = str(int(max(normalized_activations)))
        else:
            activation_count = 0
