...
SENT_SYSTEM = """You are a meticulous AI researcher conducting an important investigation into patterns found in language. Your task is to analyze text and provide an explanation that thoroughly encapsulates the possible sentence-level patterns found in it.
Guidelines:

You will be given a list of sentence examples, and for each example an overall activation value of the pattern is provided.

- Try to produce a concise final description. Simply describe the latent patterns that are common in all the sentence examples.
- If some examples appear noisy or uninformative, you may ignore them.
- Do not make lists of possible explanations. Keep your explanations short and concise.
- The last line of your response must be the formatted explanation, using [EXPLANATION]:
...
SENT_EXAMPLE_1 = """
Example 1: T cells are a type of lymphocyte that recognize antigens presented by major histocompatibility complex molecules. They play a central role in adaptive immunity by coordinating immune responses and directly killing infected cells.
Example 2: Antibodies are Y-shaped proteins produced by B cells that specifically bind to foreign antigens. They help neutralize pathogens and mark them for clearance by other components of the immune system.
Example 3: Macrophages are phagocytic immune cells that ingest and degrade pathogens, cellular debris, and dead cells. They also secrete signaling molecules that regulate inflammation and tissue repair.
"""

SENT_EXAMPLE_1_ACTIVATIONS = """
Example 1: T cells are a type of lymphocyte that recognize antigens presented by major histocompatibility complex molecules. They play a central role in adaptive immunity by coordinating immune responses and directly killing infected cells.
Activation: 9
Example 2: Antibodies are Y-shaped proteins produced by B cells that specifically bind to foreign antigens. They help neutralize pathogens and mark them for clearance by other components of the immune system.
Activation: 4
...
SENT_EXAMPLE_1_EXPLANATION = """
[EXPLANATION]: A short encyclopedic paragraph defining a component of the immune system and summarizing its primary role in host defense.
"""

SENT_EXAMPLE_2 = """
Example 1: A hash table is a data structure that maps keys to values using a hash function. It provides average-case constant-time complexity for insert, delete, and lookup operations when collisions are well controlled.
Example 2: Dijkstra’s algorithm is a graph algorithm for finding the shortest paths from a single source to all other vertices in a weighted graph with non-negative edge weights. Its typical implementation runs in O(E log V) time using a priority queue.
Example 3: A stack is a linear data structure that follows a last-in, first-out discipline for accessing elements. It supports two primary operations, push and pop, and is widely used to manage function calls and backtracking.
"""

SENT_EXAMPLE_2_ACTIVATIONS = """
Example 1: A hash table is a data structure that maps keys to values using a hash function. It provides average-case constant-time complexity for insert, delete, and lookup operations when collisions are well controlled.
Activation: 8
Example 2: Dijkstra’s algorithm is a graph algorithm for finding the shortest paths from a single source to all other vertices in a weighted graph with non-negative edge weights. Its typical implementation runs in O(E log V) time using a priority queue.
Activation: 5
...
SENT_EXAMPLE_2_EXPLANATION = """
[EXPLANATION]: A concise technical description of a core computer science data structure or algorithm, including its purpose and basic performance characteristics.
"""


SENT_EXAMPLE_3 = """
Example 1: Inflation is a sustained increase in the general price level of goods and services in an economy over time. It erodes the purchasing power of money and is commonly measured using consumer price indices.
Example 2: Gross domestic product, or GDP, is the total market value of all final goods and services produced within a country’s borders during a specific period. It is widely used as a broad indicator of economic activity and growth.
Example 3: Compound interest is the process by which interest is earned not only on the initial principal but also on accumulated interest from previous periods. Over long horizons, it can lead to exponential growth of an investment.
"""

SENT_EXAMPLE_3_ACTIVATIONS = """
Example 1: Inflation is a sustained increase in the general price level of goods and services in an economy over time. It erodes the purchasing power of money and is commonly measured using consumer price indices.
Activation: 8
Example 2: Gross domestic product, or GDP, is the total market value of all final goods and services produced within a country’s borders during a specific period. It is widely used as a broad indicator of economic activity and growth.
Activation: 9
...
SENT_EXAMPLE_3_EXPLANATION = """
[EXPLANATION]: A brief encyclopedic-style explanation of a key economic or financial concept, stating its definition and why it is important.
"""
...
def _prompt(n, activations=False, sentence_level=False, **kwargs):
    starter = (
        get(f"{'SENT_' if sentence_level else ''}EXAMPLE_{n}{'_ACTIVATIONS' if activations else ''}")
    )
...
def _response(n, cot=False, sentence_level=False, **kwargs):
    response_atoms = []
    if cot:
        response_atoms.append(get(f"EXAMPLE_{n}_COT_ACTIVATION_RESPONSE"))

    response_atoms.append(get(f"{'SENT_' if sentence_level else ''}EXAMPLE_{n}_EXPLANATION"))
...
def system(cot=False, sentence_level=False):
    prompt = ""
    ...
    return [
        {
            "role": "system",
            "content": (SENT_SYSTEM if sentence_level else SYSTEM).format(prompt=prompt),
        }
    ]
