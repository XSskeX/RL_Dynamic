...
class InMemoryCache:
    ...
    def __init__(
        ...
        batch_size: int = 64,
        activation_judge: str = "nonzero", # 'nonzero' or 'topk'
        topk: int | None = None,
    ):
        """
        ...
        self.activation_judge = activation_judge
        self.topk = topk
    ...
    def add(
        ...
        """
        if self.activation_judge == 'nonzero':
            latent_locations, latent_activations = self.get_nonzeros(latents, module_path)
        elif self.activation_judge == 'topk':
            assert self.topk is not None, "topk must be set for topk activation_judge"
            # Get the top-k activations and their indices
            topk_activations, topk_indices = torch.topk(latents, self.topk, dim=-1)

            # Create a mask for the top-k activations
            mask = torch.zeros_like(latents, dtype=torch.bool)
            mask.scatter_(-1, topk_indices, True)

            # Get the locations of the top-k activations
            latent_locations = torch.nonzero(mask)
            latent_activations = latents[mask]
...
class LatentCache:
    ...
    def __init__(
        ...
        save_router_weights_flag: bool = False,
    ):
        """
        ...
        self.batch_size = batch_size
        self.width = None
        self.save_router_weights_flag = save_router_weights_flag
        if save_router_weights_flag and router_topk is not None:
            self.router_cache = InMemoryCache(filters, batch_size=batch_size, activation_judge='topk', topk=router_topk)
        self.sparse_cache = InMemoryCache(filters, batch_size=batch_size, activation_judge='nonzero')
    ...
    def load_token_batches(
        ...
        """
        print(f"Total tokens before adjustment: {tokens.shape[0] * tokens.shape[1]}", flush=True)
        max_batches = n_tokens // tokens.shape[1]
        tokens = tokens[:max_batches]

        print(f"Total tokens after adjustment: {tokens.shape[0] * tokens.shape[1]}", flush=True)
    ...
    def run(self, n_tokens: int, tokens: token_tensor_type):
                    ...
                    with collect_activations(
                        ...
                        for hookpoint, latents in activations.items():
                            if len(latents.shape) == 2:
                                latents = latents.reshape(batch.shape[0], -1, latents.shape[-1])
                            ...
                            # import pdb; pdb.set_trace()
                            if self.save_router_weights_flag:
                                self.router_cache.add(latents, batch, batch_number, hookpoint)
                            self.sparse_cache.add(sae_latents, batch, batch_number, hookpoint)
        ...
        self.sparse_cache.concat()
        if self.save_router_weights_flag:
            self.router_cache.concat()
        self.save_firing_counts()
    
    # def save_router_weights(self, save_dir: Path, save_tokens: bool = True):
    #     """
    #     Save the cached router weights to disk.
    ...
    #         Defaults to True.
    #     """
    #     for module_path in self.router_cache.latent_locations.keys():
    #         output_file = save_dir / f"{module_path}_router_weights.safetensors"

    #         data = {
    #             "locations": self.router_cache.latent_locations[module_path].numpy(),
    #             "activations": self.router_cache.latent_activations[module_path].numpy(),
    #         }
    #         if save_tokens:
    #             data["tokens"] = self.router_cache.tokens[module_path].numpy()
    ...
    def save(self, save_dir: Path, save_tokens: bool = True):
        ...
        Args:
            ...
            Defaults to True.
        """
        for module_path in self.sparse_cache.latent_locations.keys():
            output_file = save_dir / f"{module_path}.safetensors"

            data = {
                "locations": self.sparse_cache.latent_locations[module_path].numpy(),
                "activations": self.sparse_cache.latent_activations[module_path].numpy(),
            }
            if save_tokens:
                data["tokens"] = self.sparse_cache.tokens[module_path].numpy()
    ...
    def save_splits(self, n_splits: int, save_dir: Path, save_tokens: bool = True):
        ...
        for module_path in self.sparse_cache.latent_locations.keys():
            latent_locations = self.sparse_cache.latent_locations[module_path]
            latent_activations = self.sparse_cache.latent_activations[module_path]
            tokens = self.sparse_cache.tokens[module_path].numpy()
    ...
    def generate_statistics_cache(self):
        ...
        for module_path in self.sparse_cache.latent_locations.keys():
            ...
            generate_statistics_cache(
                self.sparse_cache.tokens[module_path],
                self.sparse_cache.latent_locations[module_path],
                self.sparse_cache.latent_activations[module_path],
                self.width,
                verbose=True,
            )
