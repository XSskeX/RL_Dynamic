...
def run_latent_steering_experiment(
    ...
    results, temporal_results = latent_steering_experiment(
    ...
    save_results_to_csv(temporal_results, temporal_results_file)
    return results
...
def latent_steering_experiment(
    ...
    for model_name, model in models.items():
        ...
        for prompt_idx, prompt in enumerate(prompts):
            ...
            for batch_idx, batch_data in tqdm(
                ...
                generated_texts, temporal_generated_texts = _generate_with_steering_batched_single_mode(
                    model=model,
                    tokenizer=tokenizer,
                ...
                for config, temporal_generated_text in zip(
                    batch_data["configs"], temporal_generated_texts
                ):
                    result = {
                        "model_type": model_name,
                        "latent_idx": config["latent_idx"],
                        ...
                        "prompt": prompt,
                        "formatted_prompt": batch_data["formatted_prompt"],
                        "generated_text": temporal_generated_text,
                        "layer": layer,
                        "batch_idx": batch_idx,
                        "prompt_idx": prompt_idx,
                    }
    ...
    return results, temporal_results
...
def _generate_with_steering_batched_single_mode(
    ...
    with model.generate(
        ...
        with tracer.invoke(dict(input_ids=batch_input_ids)):
            if steering_mode == "baseline":
                # No steering applied - generate normally
                temporal_outputs = model.generator.output.save()
                print(type(temporal_outputs))
                print("-------------------------baseline-------------------------")
                pass
                ...
                print("-------------------------all_tokens-------------------------")
                with model.layers[layer].iter[:10]:
                    ...
                    #steering_additive = torch.zeros_like(steering_additive)
                    print(steering_additive.shape)
                    model.layers_output[layer][:] += steering_additive.unsqueeze(1)
                    print("successfully change model")
                temporal_outputs = model.generator.output.save()
                print(type(temporal_outputs))
                ...
                temporal_outputs = tracer.result.save()
                print(f"temporal_outputs type: {type(temporal_outputs)}")
                print(temporal_outputs)
            else:
        ...
        with tracer.invoke():
            ...
            print(f"temporal_outputs type: {type(outputs)}")
    ...
    assert (
        temporal_outputs.shape[0] == outputs.shape[0]
    ), f"Expected {outputs.shape[0]} outputs, got {temporal_outputs.shape[0]}"

    # Decode all generated texts
    generated_texts = []
    temporal_generated_texts = []
    for i in range(actual_batch_size):
        text = tokenizer.decode(outputs[i], skip_special_tokens=False)
        temporal_text = tokenizer.decode(temporal_outputs[i], skip_special_tokens=False)
        generated_texts.append(text)
        temporal_generated_texts.append(temporal_text)

    assert (
        len(generated_texts) == actual_batch_size
    ), f"Expected {actual_batch_size} generated texts, got {len(generated_texts)}"

    return generated_texts, temporal_generated_texts
